...
class RedisManager():
    db_num = {'orderbooks': 0, 'prices': 1, 'orders': 2, 'system_state': 4}

    def __init__(self, db_name: str,
                 group_name: str = None,
                 consumer_name: str = None,
                 read_existing: bool = True):
        assert db_name in ('orderbooks', 'prices', 'orders', 'system_state'
                           ), "db_name should be in ('orderbooks', 'prices', 'orders', 'system_state')"
        self.db_name = db_name
        self.redis_client = redis.Redis(db=self.db_num[db_name], decode_responses=True)

    def set_system_state(self, module, value):
        assert self.db_name == 'system_state', 'Переключитесь на system_state таблицу!'
        self.redis_client.setex(name=module, time=10, value=value)

    def get_system_state(self, module):
        assert self.db_name == 'system_state', 'Переключитесь на system_state таблицу!'
        return self.redis_client.get(module)
    ...
    def add_order(self, exchange, token, qty, price, side, action, leverage, dp, ord_id, ts, status):
        assert self.db_name == 'orders', 'Переключитесь на orders таблицу!'
        self.redis_client.hset(name=f'pending_orders:{exchange}:{token}',
                               mapping={'qty': qty, 'price': price, 'side': side, 'action': action,
